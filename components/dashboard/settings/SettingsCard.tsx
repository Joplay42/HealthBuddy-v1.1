...
const SettingsCard = ({ card }: cardProps) => {
	...
	const handleSubmit = async ({ event }: settingsCardProps) => {
		...
		if (card.title == "Appearance") {
			toggleTheme();
		} else if (card.title == "Reset password") {
			try {
	...
	return (
		<div
			id={card.id}
			className='p-5 md:p-10 bg-white dark:bg-ink-900 rounded-xl w-full space-y-2'
		>
			{/** Form container */}
			<form onSubmit={(e) => handleSubmit({ event: e })}>
				{/** Title */}
				<h1 className='text-lg md:text-xl font-semibold dark:text-bone'>
					{card.title}
				</h1>
				<p className='text-neutral-500 dark:text-white/55 pb-4'>
					{card.description}
				</p>
				<hr className='border-neutral-400 dark:border-white/10' />
				<div className='lg:flex gap-x-5 pt-4 pb-6 md:pb-10'>
					{/** If the card has an input to enter */}
					{card.input.length == 1 ? (
						<div className='flex flex-col w-full gap-y-2'>
							{/** Display the label above */}
							<label className='font-bold dark:text-bone'>
								{card.input[0]}
							</label>
							{/** Display the input */}
							<input
					...
					className={`flex items-center gap-2 justify-center py-4 px-3 rounded-xl hover:opacity-75 hover:transition ease-in-out duration-300  text-white w-full disabled:opacity-60 ${
						...
						card.title === "Delete account" && accountDelete
							...
							: card.title === "Delete account"
								? "bg-red-500"
								: "bg-black dark:bg-lime dark:text-ink-950"
					}`}
					type='submit'
					disabled={loading}
				>
